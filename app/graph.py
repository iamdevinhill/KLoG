...
def get_graph(username: str = Depends(get_current_user)):
    ...
    with d.session() as session:
        nodes_result = session.run(
            ...
            RETURN p.title AS title, count(r) AS connections, p.color AS color
            """,
            username=username,
        )
        nodes = []
        for r in nodes_result:
            nodes.append(GraphNode(id=r["title"], title=r["title"], connections=r["connections"], color=r["color"] or "#00e5ff"))
